...
public interface AbstractRestServer<Entity extends TrackableEntity> {
    ...
    JsonViewObject getPage(@ApiParam(value = "查询条件和分页参数", required = true, example = "{\"pageSize\": 10, \"pageNum\": 1}") @RequestBody Page<Entity> page);
    ...
    JsonViewObject getById(@ApiParam(value = "记录的id", required = true, example = "1") @PathParam("id") String id);
    ...
    JsonViewObject save(@ApiParam(value = "记录的JSON格式字符串", required = true) @RequestBody Entity entity);
    ...
    JsonViewObject update(@ApiParam(value = "记录的JSON格式字符串", required = true) @RequestBody Entity entity);
